class Retangulo:
    ...
    def base(self, valor):
        ...
        if valor < 0:
            raise ValueError(f"Valor inválido para a base. ({valor})")
        else:
            self._base = valor
    ...
    def altura(self, valor):
        ...
        if valor < 0:
            raise ValueError(f"Valor inválido para a altura. ({valor})")
        else:
            self._altura = valor
    ...
    def medidas(self, valores:tuple):
        ...
        if len(valores) != 2:
            raise SyntaxError("Informe uma tupla com dois valores numéricos.")
        if isinstance(valores[0], float) or isinstance(valores[1], int):
            self.base = valores[0]
        else:
            raise ValueError("A base deve ser um número.")
